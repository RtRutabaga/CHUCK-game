...
(data/choices/*.json, validated loudly, every branch cross-checked
against the dialogue file by tests), so any future decision anywhere in
the game is a JSON entry, not new code. 
