...
def test_chult_2_save_continues_and_sanity_return() -> None:
    ...
    try:
        ...
        assert game.checkpoints.write_save("chult_2", scene.sanity.current)
        assert game.active_checkpoint_id == "chult_2"
        assert game.saves.load().checkpoint_id == "chult_2"
    finally:
        game._shutdown()
    ...
    try:
        ...
        assert resumed.active_checkpoint_id == "chult_2"
    finally:
        resumed._shutdown()
        directory.cleanup()
...
def test_phase_4_boundary_enters_chult_2_without_a_transition_bounce() -> None:
    ...
    try:
        ...
        # The landing arrives in a fade; let it finish before walking.
